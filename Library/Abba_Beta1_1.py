"""
Automatic Basic Book Archiver (Abba) V1.1
...
#This function should allow the program to quit or stat the scan again from any point in the process
def tupni(message):
	global quit
...
while quit == False: #keep looping until quit
	...
	#Otherwise if the book has neither a barcode or a ISBN (usually means the book is older than ~1965) then the info is entered in manually (ugh...)
		...
		Author = tupni("Enter the name of the author:\n").title()
	...
	if Title:
		...
		subject2 = tupni("Please enter the secondary subject if any:\n").title()
		subject3 = tupni("Please enter any additional subjects or comments:\n").title()
		volume = tupni("Please enter the volume number:\n")
		donor = tupni("Please enter the donor (enter l for Ex-Libris):\n").title()
		
		if donor == "L":
			donor = "Ex-Libris"
		relevance = tupni("Is this book relevant to the physics library? (Y/N/M)").title()
