...
To do list:
Implement dynamic subject additions
...
# This function is used to allow tab to autocomplete to be used, anywhere the list tabber is used the auotcomplete is used
def completer(text, state):
    options = [x for x in tabber if x.startswith(text)]
    try:
...
def Topic(IDnumb):
	...
	if IDnumb not in TopicDict:
		user_input = raw_input("It appears that the ID number " + IDnumb + " is not assigned yet, would you like to assign it? (Y/N): ").title()
...
# 	for row in subjectfile:
# 		tabber.append(row[0])
# 	print tabber
# 	subjects = ListBuilder("Please enter additional subjects (if any), hit enter twice to carry on:")
...
while quit == False: # Keep looping until quit
	...
	try:
		...
		user_input1 = raw_input("Does the book have a Barcode (B) ISBN number (I) or neither (N): ").title()
		info = None
		tabber = []
		...
		# Otherwise if the book has neither a barcode or a ISBN (usually means the book is older than ~1965) then the info is entered in manually (ugh...effort...)
			...
			tabber = [x.lower() for x in CSVToList("Authors")]
			Author = ListBuilder("Enter the name of the author(s), hit enter twice to carry on:")	
			if Author not in tabber:
				writer = csv.writer(open('Author.csv', 'wb'), delimiter=',')
				writer.writerow(tabber + Author)
			
			tabber = [x.lower() for x in CSVToList("Publisher")]
			Publisher = raw_input("Enter the publishers:\n").title()
			if Publisher not in tabber:
				writer = csv.writer(open('Publisher.csv', 'wb'), delimiter=',')
				writer.writerow(tabber + Publisher)
		...
		if Title:
			...
			IDnumb = raw_input("Does the book have a Library ID Number? If so what is it? (Hit enter for no ID): ").title()

			if IDnumb:
				subject1 = Topic(IDnumb)
				tabber = [x.lower() for x in CSVToList("Subjects")]
				print "Previously assigned subjects:" 
				print [x.title() for x in tabber]
				subjects = ListBuilder("Please enter additional subjects (if any), hit enter twice to carry on:")
				subjects = subject1 + "; " + subjects
				for i in range(len(subjects)):
					if subjects[i] not in tabber:
						writer = csv.writer(open('subjects.csv', 'wb'), delimiter=',')
						writer.writerow(tabber + subjects)
			else:
				subject1 = raw_input("Enter the main subject of the book:").title()
				tabber = [x.lower() for x in CSVToList("Subjects")]
				print "Previously assigned subjects:" 
				print [x.title() for x in tabber]
				subjects = ListBuilder("Please enter additional subjects (if any), hit enter twice to carry on:")
			...
			if Donor == "L":
				Donor = "Ex-Libris"
			Relevance = raw_input("Is this book relevant to the physics library? (Y/N/M)").title()

			Comment = raw_input("Please enter in any additional comments about the book: ").title()
			...
			check = raw_input("Is this information correct?(Y/N): ").title()
			...
			if check == "N":
				...
				user_inputx = raw_input("You have so far scanned books at a rate of " + str(int(bookrate)) + " sec/book (~"+str(int(dayrate))+" books a day).\nWould you like to scan another? (Y/N): ").title()
