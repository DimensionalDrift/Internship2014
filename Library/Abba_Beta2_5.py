...
Beta 2.5 	- Added the 100 book quota
	...
	try:
		...
		if user_input1 == "B":
			...
			while not info: # Continue scan until the information has been assigned
				...
				if info:
					...
					print "Year:" + Year
					print "\nIf the information is not correct, use Ctrl^C to cancel\n"
		...
		if Title:
			...
			if check == "N":
				...
				dayrate = 3600/bookrate
				if bookno == 100:
					print "Congatualtions! You have finally scanned your quota for the day!"
				user_inputx = tupni("You have so far scanned "+str(bookno)+" ("+str(100-bookno)+" left to go!) books at a rate of " + str(int(bookrate)) + " sec/book (~"+str(int(dayrate))+" books an hour).\nWould you like to scan another? (Y/N): ", None)
